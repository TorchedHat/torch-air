# vLLM Hardware Accelerator Integration Readiness Checklist

> A comprehensive template for evaluating out-of-tree hardware accelerator
> integration readiness with vLLM's plugin architecture.
>
> Derived from vLLM's official [Plugin System docs][plugin-docs],
> [Platform API reference][platform-api], [Custom Op docs][customop-docs],
> [Attention Backend docs][attn-docs], real-world backends (vllm-ascend, HPU, TPU),
> and adversarially verified against vLLM source code.

**Backend under evaluation**: _[FILL: backend name]_
**Evaluation date**: _[FILL: date]_
**Evaluator**: _[FILL: human or agent]_
**vLLM version**: _[FILL: version]_
**PyTorch readiness score**: _[FILL: X.X / 10 from PyTorch checklist]_

**Status markers**: `[ ]` Not Ready | `[~]` Partially Ready | `[x]` Ready | `[N/A]` Not applicable

**Important caveats**:
- vLLM has not reached 1.0; all plugin interfaces are subject to change.
- Platform class methods raise `NotImplementedError` by convention, not `@abstractmethod` -- failures occur at runtime, not class definition time.
- vLLM is migrating from v0 to v1 architecture; this checklist primarily reflects v1.

---

## Readiness Score & Summary

### Executive Summary

_[FILL: Write a concise summary covering:
- **Overall verdict** and score (X.X / 10)
- **Key strengths**:
  - Top strength 1
  - Top strength 2
  - Top strength 3
- **Key gaps**:
  - Critical gap 1
  - Critical gap 2
  - Critical gap 3
- **Key next steps**:
  - Highest-impact action 1
  - Highest-impact action 2
  - Highest-impact action 3
- **Notable insights**:
  - Surprising finding or important context 1
  - Surprising finding or important context 2]_

### Section Scores

| Section | Rank | Max Pts | Earned | Pct | Weighted |
|---------|------|---------|--------|-----|----------|
| PyTorch Prerequisites | 1 | | | | |
| Plugin Registration | 1 | | | | |
| Platform Class | 1 | | | | |
| Attention Backend | 1 | | | | |
| Worker | 1 | | | | |
| Custom Ops | 1 | | | | |
| Model Runner | 2 | | | | |
| KV Cache Management | 2 | | | | |
| Memory Management | 2 | | | | |
| Distributed | 2 | | | | |
| Model Compatibility | 2 | | | | |
| Dtype Support | 2 | | | | |
| Compilation / Graph Capture | 3 | | | | |
| Testing & CI | 3 | | | | |
| Quantization | 3 | | | | |
| Speculative Decoding | 3 | | | | |
| Multimodal | 3 | | | | |
| Profiling | 3 | | | | |
| **TOTAL** | | | | | |

### Calculation

```
Row scoring:
  3 pts = Critical, 2 pts = Important, 1 pt = Nice-to-have
  [x] = full points, [~] = half points, [ ] = 0, [N/A] = excluded from max

Section score:
  section_pct = earned_pts / max_pts

Overall score formula:
  weight_r = 1 / rank
  Overall Score = (sum(section_pct * weight_r) / sum(weight_r)) * 10

where the sums are over all applicable sections (excluding fully N/A sections).
```

### Final Verdict

| Score | Verdict |
|-------|---------|
| 0 - 4 | **Not Ready** |
| 4 - 7 | **Partially Ready** |
| 7 - 10 | **Ready** |

**Overall Score**: _____ / 10
**Verdict**: _____

### Summary

_[FILL: Write as bullet points:
- **Score**: X.X/10 (Verdict)
- **Production-quality areas**: List fully implemented areas
- **Attention**: Op count and supported features
- **Model compatibility**: Architecture count and highlights
- **Gaps**:
  - Gap 1
  - Gap 2
- **Next steps**:
  - Action 1
  - Action 2
- **Character**: One line on plugin's architecture and approach]_

---

## Scoring Model

Each row has a **point value** (1-3) reflecting its importance:
- **3 pts** = Critical (blocks basic functionality)
- **2 pts** = Important (expected for production use)
- **1 pt** = Nice-to-have (polish, edge cases)

Each section has a **rank** (1-3) reflecting its criticality tier for vLLM accelerator integration:
- **Rank 1** = Foundational (blocks basic functionality)
- **Rank 2** = Core Production (expected for production use)
- **Rank 3** = Quality of Life (polish, ecosystem, sustainability)

**Row scoring**: `[x]` = full points, `[~]` = half points, `[ ]` = 0, `[N/A]` = excluded from max

**Section score** = `section_pct = earned_pts / max_pts`

**Overall score**:
```
weight_r = 1 / rank
Overall Score = (sum(section_pct * weight_r) / sum(weight_r)) * 10
```
where the sums are over all applicable sections (excluding fully N/A sections).

**Readiness verdict**:
- **0 - 4** = Not Ready
- **4 - 7** = Partially Ready
- **7 - 10** = Ready

---

## 0. PyTorch Backend Prerequisites -- Rank: **1**

vLLM is built on PyTorch. A functional PyTorch backend is a hard prerequisite.
Evaluate using the [PyTorch Accelerator Readiness Checklist](pytorch_checklist.md) first.

| # | Item | Pts | Status | Notes |
|---|------|-----|--------|-------|
| 0.1 | PyTorch PrivateUse1 backend registered and functional | 3 | `[ ]` | |
| 0.2 | `torch.device("<name>")` works | 3 | `[ ]` | |
| 0.3 | Tensor creation on device (`torch.empty`, `torch.zeros`) | 3 | `[ ]` | |
| 0.4 | Device<->CPU transfer (`.to()`, `.cpu()`) | 3 | `[ ]` | |
| 0.5 | Core ATen operators (add, mm, softmax, etc.) | 3 | `[ ]` | |
| 0.6 | `torch.compile` works with device | 2 | `[ ]` | |
| 0.7 | `torch.distributed` backend registered (for TP/PP) | 3 | `[ ]` | |
| 0.8 | Autograd backward pass works | 1 | `[ ]` | |
| 0.9 | AMP / autocast works (for mixed-precision inference) | 2 | `[ ]` | |
| 0.10 | Serialization (`torch.save`/`torch.load`) works | 2 | `[ ]` | |
| 0.11 | PyTorch readiness score | 3 | `[ ]` | |

---

## Structure

vLLM organizes accelerator integration around four core abstractions:

1. **Platform** -- Device management, capability detection, configuration validation
2. **Worker** -- Device initialization, memory management, model loading, KV cache
3. **ModelRunner** -- Model execution, input preparation, sampling
4. **AttentionBackend** -- Attention computation, KV cache layout, paged attention

Plus cross-cutting concerns: CustomOp registration, distributed communication,
compilation/graph capture, and quantization support.

---

## 1. Platform Plugin Registration -- Rank: **1**

| # | Item | Pts | Status | Notes |
|---|------|-----|--------|-------|
| 1.1 | `entry_points` registered under `vllm.platform_plugins` group | 3 | `[ ]` | |
| 1.2 | Register function returns the Platform class's fully qualified name | 3 | `[ ]` | |
| 1.3 | Register function is re-entrant (safe across multiple processes) | 2 | `[ ]` | |
| 1.4 | `vllm.general_plugins` entry point registered (if custom ops/models needed) | 2 | `[ ]` | |
| 1.5 | Platform activates correctly (OOT takes priority over built-in) | 3 | `[ ]` | |
| 1.6 | Plugin loads without error on `import vllm` | 3 | `[ ]` | |

---

## 2. Platform Class (inherits `vllm.platforms.interface.Platform`) -- Rank: **1**

### Required Class Variables

| # | Item | Pts | Status | Notes |
|---|------|-----|--------|-------|
| 2.1 | `_enum` property set (typically `PlatformEnum.OOT`) | 3 | `[ ]` | |
| 2.2 | `device_type` property returns PyTorch device type string | 3 | `[ ]` | |
| 2.3 | `device_name` class variable set | 3 | `[ ]` | |
| 2.4 | `dispatch_key` set (default `'CPU'`; override for custom dispatch) | 2 | `[ ]` | |
| 2.5 | `ray_device_key` set (for Ray resource scheduling) | 2 | `[ ]` | |
| 2.6 | `device_control_env_var` set (e.g., `"ASCEND_RT_VISIBLE_DEVICES"`) | 2 | `[ ]` | |
| 2.7 | `dist_backend` set (distributed backend name) | 2 | `[ ]` | |
| 2.8 | `supported_quantization` list defined | 2 | `[ ]` | |

### Device Management Methods

| # | Method | Pts | Status | Notes |
|---|--------|-----|--------|-------|
| 2.9 | `get_device_name()` | 3 | `[ ]` | |
| 2.10 | `get_current_memory_usage()` | 3 | `[ ]` | |
| 2.11 | `set_device(device)` | 3 | `[ ]` | |
| 2.12 | `manual_seed_all(seed)` | 1 | `[ ]` | |
| 2.13 | `check_if_supports_dtype(dtype)` | 2 | `[ ]` | |
| 2.14 | `stateless_init_device_torch_dist_pg()` | 2 | `[ ]` | |
| 2.15 | `num_compute_units()` | 1 | `[ ]` | |

### Component Getter Methods

| # | Method | Pts | Status | Notes |
|---|--------|-----|--------|-------|
| 2.16 | `get_attn_backend_cls()` returns attention backend FQN | 3 | `[ ]` | |
| 2.17 | `get_device_communicator_cls()` returns communicator FQN | 3 | `[ ]` | |
| 2.18 | `get_worker_cls()` returns worker FQN | 3 | `[ ]` | |
| 2.19 | `get_model_runner_cls()` returns model runner FQN | 3 | `[ ]` | |

### Compilation & Graph Capture Methods

| # | Method | Pts | Status | Notes |
|---|--------|-----|--------|-------|
| 2.20 | `get_compile_backend()` (default: `'inductor'`) | 2 | `[ ]` | |
| 2.21 | `get_static_graph_wrapper_cls()` | 2 | `[ ]` | |
| 2.22 | `get_pass_manager_cls()` | 1 | `[ ]` | |
| 2.23 | `support_static_graph_mode()` | 2 | `[ ]` | |

### Configuration Validation Methods

| # | Method | Pts | Status | Notes |
|---|--------|-----|--------|-------|
| 2.24 | `check_and_update_config(vllm_config)` | 3 | `[ ]` | |
| 2.25 | `verify_quantization(quant)` | 2 | `[ ]` | |
| 2.26 | `verify_model_arch(model_arch)` | 2 | `[ ]` | |

### Feature Support Methods

| # | Method | Pts | Status | Notes |
|---|--------|-----|--------|-------|
| 2.27 | `use_custom_allreduce()` | 1 | `[ ]` | |
| 2.28 | `use_custom_op_collectives()` | 2 | `[ ]` | |
| 2.29 | `supports_v1(model_config)` | 2 | `[ ]` | |

---

## 3. Worker Implementation (inherits `WorkerBase`) -- Rank: **1**

| # | Item | Pts | Status | Notes |
|---|------|-----|--------|-------|
| 3.1 | `init_device()` -- initialize device context | 3 | `[ ]` | |
| 3.2 | `initialize_cache(kv_cache_config)` -- allocate KV cache buffers | 3 | `[ ]` | |
| 3.3 | `load_model()` -- load model weights onto device | 3 | `[ ]` | |
| 3.4 | `get_kv_cache_spec()` -- return KV cache specification | 3 | `[ ]` | |
| 3.5 | `determine_available_memory()` -- report free device memory | 3 | `[ ]` | |
| 3.6 | `initialize_from_config(vllm_config)` -- full initialization | 2 | `[ ]` | |
| 3.7 | `compile_or_warm_up_model()` -- compilation/warmup step | 2 | `[ ]` | |
| 3.8 | `execute_model(scheduler_output)` -- run one forward step | 3 | `[ ]` | |

---

## 4. Model Runner Implementation (inherits `ModelRunnerBase`) -- Rank: **2**

| # | Item | Pts | Status | Notes |
|---|------|-----|--------|-------|
| 4.1 | Input preparation (tokenized inputs -> device tensors) | 3 | `[ ]` | |
| 4.2 | Forward pass execution | 3 | `[ ]` | |
| 4.3 | Sampling / output processing | 3 | `[ ]` | |
| 4.4 | `InputBatch` handling | 2 | `[ ]` | |
| 4.5 | `SamplingMetadata` construction | 2 | `[ ]` | |
| 4.6 | Multi-step scheduling support | 1 | `[ ]` | |

---

## 5. Attention Backend (inherits `AttentionBackend`) -- Rank: **1**

| # | Item | Pts | Status | Notes |
|---|------|-----|--------|-------|
| 5.1 | Attention backend class implemented | 3 | `[ ]` | |
| 5.2 | Registered via `get_attn_backend_cls()` in Platform | 3 | `[ ]` | |
| 5.3 | `validate_configuration()` checks compatibility | 2 | `[ ]` | |
| 5.4 | Paged attention support | 3 | `[ ]` | |
| 5.5 | Prefill attention (variable-length) | 3 | `[ ]` | |
| 5.6 | Decode attention (single-token) | 3 | `[ ]` | |

### Attention Features

| Feature | Pts | Status | Notes |
|---------|-----|--------|-------|
| Chunked prefill | 3 | `[ ]` | |
| Prefix caching (APC) | 2 | `[ ]` | |
| Sliding window attention | 1 | `[ ]` | |
| Multi-head attention (MHA) | 3 | `[ ]` | |
| Grouped-query attention (GQA) | 3 | `[ ]` | |
| Multi-query attention (MQA) | 2 | `[ ]` | |
| Multi-latent attention (MLA/DeepSeek) | 2 | `[ ]` | |
| FP8 KV cache | 1 | `[ ]` | |
| Speculative decoding attention | 2 | `[ ]` | |
| Encoder-decoder cross-attention | 1 | `[ ]` | |

---

## 6. KV Cache Management -- Rank: **2**

| # | Item | Pts | Status | Notes |
|---|------|-----|--------|-------|
| 6.1 | `update_block_size_for_backend()` implemented | 3 | `[ ]` | |
| 6.2 | Block allocation on device | 3 | `[ ]` | |
| 6.3 | Block copy (CoW / fork) | 2 | `[ ]` | |
| 6.4 | Block swap (device <-> CPU) | 2 | `[ ]` | |
| 6.5 | `support_hybrid_kv_cache()` | 1 | `[ ]` | |
| 6.6 | `register_custom_kv_cache_specs()` | 1 | `[ ]` | |
| 6.7 | Prefix caching hash-based block reuse | 2 | `[ ]` | |

---

## 7. Custom Ops Registration -- Rank: **1**

| # | Item | Pts | Status | Notes |
|---|------|-----|--------|-------|
| 7.1 | `@CustomOp.register_oot('op_name')` decorator used | 3 | `[ ]` | |
| 7.2 | `forward_oot()` method implemented for OOT dispatch | 3 | `[ ]` | |
| 7.3 | Key ops: `paged_attention` | 3 | `[ ]` | |
| 7.4 | Key ops: `rotary_embedding` | 3 | `[ ]` | |
| 7.5 | Key ops: `rms_norm` / `layer_norm` | 3 | `[ ]` | |
| 7.6 | Key ops: `silu_and_mul` / activation fusions | 2 | `[ ]` | |
| 7.7 | Key ops: `fused_moe` (for MoE models) | 2 | `[ ]` | |
| 7.8 | Key ops: `all_reduce` / collective ops | 3 | `[ ]` | |

---

## 8. Quantization Support -- Rank: **3**

| # | Item | Pts | Status | Notes |
|---|------|-----|--------|-------|
| 8.1 | `supported_quantization` list in Platform class | 3 | `[ ]` | |
| 8.2 | `verify_quantization(quant)` validates compatibility | 2 | `[ ]` | |

### Quantization Methods

| Method | Pts | Status | Notes |
|--------|-----|--------|-------|
| FP8 (W8A8) | 3 | `[ ]` | |
| GPTQ (W4A16) | 2 | `[ ]` | |
| AWQ (W4A16) | 2 | `[ ]` | |
| SqueezeLLM | 1 | `[ ]` | |
| INT8 (W8A8 smoothquant) | 2 | `[ ]` | |
| GGUF | 1 | `[ ]` | |
| BitsAndBytes (NF4) | 1 | `[ ]` | |
| Marlin (optimized GPTQ) | 1 | `[ ]` | |

---

## 9. Distributed Communication -- Rank: **2**

| # | Item | Pts | Status | Notes |
|---|------|-----|--------|-------|
| 9.1 | `DeviceCommunicatorBase` subclass implemented | 3 | `[ ]` | |
| 9.2 | Registered via `get_device_communicator_cls()` in Platform | 3 | `[ ]` | |
| 9.3 | `dist_backend` set in Platform class | 2 | `[ ]` | |

### Collective Operations

| Collective | Pts | Status | Notes |
|-----------|-----|--------|-------|
| `all_reduce` | 3 | `[ ]` | |
| `all_gather` | 2 | `[ ]` | |
| `reduce_scatter` | 2 | `[ ]` | |
| `broadcast` | 2 | `[ ]` | |
| `barrier` | 1 | `[ ]` | |
| `send` / `recv` (P2P) | 1 | `[ ]` | |

### Parallelism Support

| # | Item | Pts | Status | Notes |
|---|------|-----|--------|-------|
| 9.4 | Tensor parallelism (TP) | 3 | `[ ]` | |
| 9.5 | Pipeline parallelism (PP) | 2 | `[ ]` | |
| 9.6 | Expert parallelism (EP, for MoE) | 2 | `[ ]` | |
| 9.7 | Data parallelism (DP, multi-instance) | 1 | `[ ]` | |

---

## 10. Compilation & Graph Capture -- Rank: **3**

| # | Item | Pts | Status | Notes |
|---|------|-----|--------|-------|
| 10.1 | `torch.compile` works with backend (via `get_compile_backend()`) | 3 | `[ ]` | |
| 10.2 | Static graph capture / CUDA Graph equivalent | 3 | `[ ]` | |
| 10.3 | `get_static_graph_wrapper_cls()` returns wrapper | 2 | `[ ]` | |
| 10.4 | `support_static_graph_mode()` returns True | 2 | `[ ]` | |
| 10.5 | Piecewise compilation support (compilation levels 0-3) | 1 | `[ ]` | |
| 10.6 | Custom pass manager (via `get_pass_manager_cls()`) | 1 | `[ ]` | |

---

## 11. Memory Management -- Rank: **2**

| # | Item | Pts | Status | Notes |
|---|------|-----|--------|-------|
| 11.1 | `determine_available_memory()` returns accurate free memory | 3 | `[ ]` | |
| 11.2 | `get_current_memory_usage()` tracks current allocation | 3 | `[ ]` | |
| 11.3 | GPU memory fraction limiting (`gpu_memory_utilization`) | 2 | `[ ]` | |
| 11.4 | Memory profiling for auto KV cache sizing | 2 | `[ ]` | |
| 11.5 | Swap space (CPU offload) for KV cache | 1 | `[ ]` | |

---

## 12. Speculative Decoding -- Rank: **3**

| # | Item | Pts | Status | Notes |
|---|------|-----|--------|-------|
| 12.1 | Draft model execution on device | 3 | `[ ]` | |
| 12.2 | Verification step on device | 3 | `[ ]` | |
| 12.3 | Token acceptance/rejection logic | 2 | `[ ]` | |
| 12.4 | Bonus token handling | 1 | `[ ]` | |

---

## 13. Multimodal Support -- Rank: **3**

| # | Item | Pts | Status | Notes |
|---|------|-----|--------|-------|
| 13.1 | Image encoder execution on device | 2 | `[ ]` | |
| 13.2 | Video encoder execution on device | 1 | `[ ]` | |
| 13.3 | Audio encoder execution on device | 1 | `[ ]` | |
| 13.4 | Cross-attention for encoder-decoder models | 2 | `[ ]` | |
| 13.5 | Multimodal input preprocessing pipeline | 2 | `[ ]` | |

---

## 14. Model Compatibility -- Rank: **2**

| # | Item | Pts | Status | Notes |
|---|------|-----|--------|-------|
| 14.1 | `verify_model_arch(model_arch)` validates supported models | 2 | `[ ]` | |
| 14.2 | Decoder-only models (LLaMA, GPT, Mistral, etc.) | 3 | `[ ]` | |
| 14.3 | Encoder-decoder models (T5, BART, etc.) | 1 | `[ ]` | |
| 14.4 | MoE models (Mixtral, DeepSeek-MoE) | 2 | `[ ]` | |
| 14.5 | Multi-latent attention models (DeepSeek-V2/V3) | 2 | `[ ]` | |
| 14.6 | LoRA adapter support | 2 | `[ ]` | |

---

## 15. Profiling & Observability -- Rank: **3**

| # | Item | Pts | Status | Notes |
|---|------|-----|--------|-------|
| 15.1 | Device-side profiling (torch.profiler or equivalent) | 2 | `[ ]` | |
| 15.2 | Memory usage reporting | 2 | `[ ]` | |
| 15.3 | Throughput metrics (tokens/sec) | 2 | `[ ]` | |
| 15.4 | Latency metrics (TTFT, TPOT, ITL) | 2 | `[ ]` | |
| 15.5 | Custom stat logger via `vllm.stat_logger_plugins` | 1 | `[ ]` | |

---

## 16. Testing & Validation -- Rank: **3**

### Functional Tests

| Test Area | Pts | Status | Notes |
|-----------|-----|--------|-------|
| Basic inference (single request) | 3 | `[ ]` | |
| Batched inference | 3 | `[ ]` | |
| Streaming output | 2 | `[ ]` | |
| Greedy decoding correctness | 3 | `[ ]` | |
| Sampling (top-k, top-p, temperature) | 2 | `[ ]` | |
| Beam search | 1 | `[ ]` | |
| Parallel decoding | 1 | `[ ]` | |
| Long context (>4K tokens) | 2 | `[ ]` | |
| Stop sequences / stop tokens | 1 | `[ ]` | |

### Integration Tests

| Test Area | Pts | Status | Notes |
|-----------|-----|--------|-------|
| OpenAI-compatible API server | 3 | `[ ]` | |
| Tensor parallel inference | 2 | `[ ]` | |
| Pipeline parallel inference | 1 | `[ ]` | |
| Quantized model inference | 2 | `[ ]` | |
| Multimodal inference | 1 | `[ ]` | |
| LoRA serving | 1 | `[ ]` | |

### CI Integration

| # | Item | Pts | Status | Notes |
|---|------|-----|--------|-------|
| 16.1 | CI pipeline runs on target hardware | 3 | `[ ]` | |
| 16.2 | Regression tests on each vLLM update | 2 | `[ ]` | |
| 16.3 | Model correctness benchmarks | 2 | `[ ]` | |
| 16.4 | Performance benchmarks (throughput, latency) | 1 | `[ ]` | |

---

## 17. Dtype Support -- Rank: **2**

| Dtype | Pts | Compute | KV Cache | Quantization | Notes |
|-------|-----|---------|----------|-------------|-------|
| `float16` | 3 | `[ ]` | `[ ]` | -- | |
| `bfloat16` | 3 | `[ ]` | `[ ]` | -- | |
| `float32` | 2 | `[ ]` | `[ ]` | -- | |
| `float8_e4m3fn` | 2 | `[ ]` | `[ ]` | `[ ]` | |
| `float8_e5m2` | 1 | `[ ]` | `[ ]` | `[ ]` | |
| `int8` | 2 | `[ ]` | -- | `[ ]` | |
| `int4` | 1 | -- | -- | `[ ]` | |

---

## 18. API & Registration Quick Reference

| What | API / Mechanism | Where |
|------|----------------|-------|
| Platform plugin | `entry_points = {"vllm.platform_plugins": ["name = pkg:register"]}` | setup.py |
| General plugin | `entry_points = {"vllm.general_plugins": ["name = pkg:register"]}` | setup.py |
| Platform class | Inherit `vllm.platforms.interface.Platform` | Plugin package |
| Worker class | Inherit `vllm.v1.worker.worker_base.WorkerBase` | Plugin package |
| Model runner | Inherit `vllm.v1.worker.model_runner_base.ModelRunnerBase` | Plugin package |
| Attention backend | Implement attention backend interface | Plugin package |
| Communicator | Inherit `DeviceCommunicatorBase` | Plugin package |
| Custom ops | `@CustomOp.register_oot('op_name')` + `forward_oot()` | Plugin package |
| Quantization | Add to `supported_quantization` list in Platform | Platform class |
| Compile backend | Override `get_compile_backend()` in Platform | Platform class |

---

## Sources

- [vLLM Plugin System Documentation][plugin-docs]
- [Platform API Reference][platform-api]
- [Custom Op Documentation][customop-docs]
- [Attention Backend Documentation][attn-docs]
- [vLLM Hardware Plugin Blog Post][blog]
- [vllm-ascend Reference Implementation][vllm-ascend]
- [Hardware Plugin RFC #11162][rfc-11162]
- [API Stability RFC #19161][rfc-19161]

[plugin-docs]: https://docs.vllm.ai/en/latest/design/plugin_system/
[platform-api]: https://docs.vllm.ai/en/latest/api/vllm/platforms/interface.html
[customop-docs]: https://docs.vllm.ai/en/stable/design/custom_op/
[attn-docs]: https://docs.vllm.ai/en/latest/design/attention_backends/
[blog]: https://vllm.ai/blog/2025-05-12-hardware-plugin
[vllm-ascend]: https://github.com/vllm-project/vllm-ascend
[rfc-11162]: https://github.com/vllm-project/vllm/issues/11162
[rfc-19161]: https://github.com/vllm-project/vllm/issues/19161
